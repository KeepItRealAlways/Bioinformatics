fun main(args: Array<String>) {
    ...
    val dna = arrayOfNulls<String>(t)

    for (i in 0 until t) {
        dna[i] = readLine()!!
    }

    val out = greedyMotifSearchWithPseudocounts(k, t, dna)
    for (i in out!!.indices) {
        println(out[i])
    }
}

fun greedyMotifSearchWithPseudocounts(k: Int, t: Int, dna: Array<String?>): Array<String?>? {
    ...
    for (m in 0..dna[0]!!.length - k) {
        ...
        for (i in 1 until t) {
            ...
            val profile = profileFromMotifsWithPseudocounts(prevMotifs)

            motifs[i] = profileMostProbableKmer(dna[i]!!, k, profile)
        }
        val currScore = scoreMotifsWithPseudocounts(motifs)
...
fun profileFromMotifsWithPseudocounts(motifs: Array<String?>): Array<DoubleArray> {
    ...
    for (i in profile.indices) {
        for (j in 0 until profile[0].size) {
            ++profile[i][j]
            profile[i][j] = profile[i][j] / (motifs.size + 4).toDouble()
        }
    }
    return profile
}
...
fun scoreMotifsWithPseudocounts(motifs: Array<String?>): Int {
    val profile = profileFromMotifsWithPseudocounts(motifs)
    var consensus = ""
